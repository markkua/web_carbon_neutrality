...
"""
    Convert source statistic data (.csv) to geojson 
        Calculate the change value compared to previous year
...
output_geojson_filename = "../pre_processed_data/ghg/co2_change_country_2000-2020.geojson"
...
for i, feature in enumerate(world_boundary_json['features']):
    ...
    for year in selected_years:
        # assign data
        if iso in emission_df[ISO_FILED_NAME].unique()\
            and year in country_df[YEAR_FIELD_NAME].to_list()\
            and year - 1 in country_df[YEAR_FIELD_NAME].to_list():
            # total
            _line_current = country_df[(country_df[YEAR_FIELD_NAME] == year)].copy()
            _line_previous = country_df[(country_df[YEAR_FIELD_NAME] == year - 1)].copy()
            _current = _line_current[CO2_FIELD_NAME].values[0]
            _previous = _line_previous[CO2_FIELD_NAME].values[0]
            _value = _current - _previous
            _value = _value if not math.isnan(_value) else None
            feature['properties'][f'co2_change_total_{year}'] = _value

            _line_current[f'minus_previous_year_total'] = _value  # for debug only
            temp_df = temp_df.append(_line_current)

            # per_capita
            _current = _line_current[CO2_PER_CAP_FIELD_NAME].values[0]
            _previous = _line_previous[CO2_PER_CAP_FIELD_NAME].values[0]
            _value = _current - _previous
            _value = _value if not math.isnan(_value) else None
            feature['properties'][f'co2_change_per_capita_{year}'] = _value

            _line_current[f'minus_previous_year_per_capita'] = _value  # for debug only
            temp_df = temp_df.append(_line_current)
            ...
            feature['properties'][f'co2_change_total_{year}'] = None
            feature['properties'][f'co2_change_per_capita_{year}'] = None
...
temp_df['minus_previous_year_total'].hist()
plt.show()
temp_df['minus_previous_year_per_capita'].hist()
plt.show()
