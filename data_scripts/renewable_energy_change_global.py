...
output_geojson_filename = "../pre_processed_data/renewable/renewable_change_2000-2020.geojson"

# source data definition
source_data_filename = "../pre_processed_data/renewable/annual-change-renewables.csv"
percentage_filename = "../pre_processed_data/renewable/annual-percentage-change-renewables.csv"

#%% Merge dataframes
source_df = pd.read_csv(source_data_filename, header=0)
percentage_df = pd.read_csv(percentage_filename, header=0)

source_df = pd.merge(source_df, percentage_df, how="left", on=["Entity", "Code", "Year"])

#%%
field_correspondence = {
    'Renewables': 'renewable_total',
    'Percentage': 'renewable_percentage',
}
# CO2_FIELD_NAME = 'Renew'
...
for i, feature in enumerate(world_boundary_json['features']):
    ...
    country_df = source_df[source_df[ISO_FILED_NAME] == iso].sort_values(YEAR_FIELD_NAME)

    for year in selected_years:
        # assign data
        if iso in source_df[ISO_FILED_NAME].unique() and year in country_df[YEAR_FIELD_NAME].to_list():
...
print(f"added {len(matched_iso_dict[2020])} out of {len(source_df[ISO_FILED_NAME].unique())} data")

# %%
import matplotlib.pyplot as plt
source_df['Renewables'].hist()
plt.show()
source_df['Percentage'].hist()
plt.show()
